...
unsigned char* mapping_function(const unsigned int num); //given an input number, find it's output string
...
void create_map(const unsigned char *file_path) {
	...
	while(!feof(file)) { //read resource file line by line
		stop = 0;
		for (unsigned int a = 0; a < 62 && !stop; a++) {
			for (unsigned int b = 0; b < 62 && !stop; b++) {
				for (unsigned int c = 0; c < 62 && !stop; c++) {
					if (a * 62 * 62 + b * 62 + c == A) { //a match is found, create map entries
						code_map[get_index(B[0])] = a;
						code_map[get_index(B[1])] = b;
						code_map[get_index(B[2])] = c;
						stop = 1; //early stop, go to next input immediately
					}
				}
			}
		}
...
unsigned char* mapping_function(const unsigned int num) {
	...
	strcpy(output, "???");
	for (unsigned int a = 0; a < 62; a++) {
		for (unsigned int b = 0; b < 62; b++) {
			for (unsigned int c = 0; c < 62; c++) {
				if (a * 62 * 62 + b * 62 + c == num) { //a match is found
	...
	return output; //when number is out of the range
}
